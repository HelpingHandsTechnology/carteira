...
export const RecentPost = () => {
  ...
  const { data: recentPost, isPending: isLoadingPosts } = useQuery({
    ...
    queryFn: () => client.api.posts.get(),
  })

  const { mutate: createPost, isPending } = useMutation({
    mutationFn: ({ name }: { name: string }) => client.api.posts.post({ name }),
    onSuccess: () => {
  ...
  return (
    <div className="w-full max-w-sm backdrop-blur-lg bg-black/15 px-8 py-6 rounded-md text-zinc-100/75 space-y-2">
      {isLoadingPosts ? (
        ...
        <p className="text-[#ececf399] text-base/6">Your recent post: "{recentPost.data.name}"</p>
      ) : (
